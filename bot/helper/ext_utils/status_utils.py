...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    msg = f"<b><a href='https://t.me/KSLEECHUPDATES'>Powerd By\nKS PREMIUM LEECH</a></b>\n\n"
    button = None
    ...
    for index, task in enumerate(
        ...
        # Task Box Start
        msg += f"<b>┌ 📊 {get_progress_bar_string(progress)}</b>\n"
        ...
        if (tstatus not in [MirrorStatus.STATUS_SEED, MirrorStatus.STATUS_QUEUEUP] and task.listener.progress):
            msg += f"<b>├ 🔄 Process :</b> {progress}\n"
            
            subsize = f" / {get_readable_file_size(task.listener.subsize)}" if task.listener.subname else ""
            msg += f"<b>├ 📦 Processed :</b> <i>{task.processed_bytes()}{subsize} of {task.size()}</i>\n"
            msg += f"<b>├ 📍 Status :</b> <b>{tstatus}...⬘</b>\n"
            msg += f"<b>├ ⚡ Speed :</b> <i>{task.speed()}</i>\n"
            msg += f"<b>├ ⏱️ Time :</b> <i>{task.eta()} of {get_readable_time(elapsed + get_raw_time(task.eta()))} ( {get_readable_time(elapsed)} )</i>\n"
            
            if tstatus == MirrorStatus.STATUS_DOWNLOAD and (task.listener.is_torrent or task.listener.is_qbit):
                try:
                    msg += f"<b>├ 👥 Peers :</b> S: {task.seeders_num()} | L: {task.leechers_num()}\n"
                except:
                    pass
        elif tstatus == MirrorStatus.STATUS_SEED:
            msg += f"<b>├ 📦 Size :</b> <i>{task.size()}</i> | <b>Uploaded :</b> <i>{task.uploaded_bytes()}</i>\n"
            msg += f"<b>├ 📍 Status :</b> <b>Seeding...🌱</b>\n"
            msg += f"<b>├ ⚡ Speed :</b> <i>{task.seed_speed()}</i> | <b>Ratio :</b> <i>{task.ratio()}</i>\n"
        else:
            msg += f"<b>├ 📦 Size :</b> <i>{task.size()}</i>\n"
            msg += f"<b>├ 📍 Status :</b> <b>{tstatus}</b>\n"

        msg += f"<b>├ 🛠️ Engine :</b> <i>{task.engine}</i>\n"
        msg += f"<b>├ 📥 In Mode :</b> #{task.listener.mode[0]}\n"
        msg += f"<b>├ 📤 Out Mode :</b> #{task.listener.mode[1]}\n"
        msg += f"<b>├ 👤 User :</b> {task.listener.message.from_user.first_name}\n"
        msg += f"<b>├ 🆔 ID :</b> <code>{task.listener.message.from_user.id}</code>\n"
        
        if task.listener.is_super_chat:
            msg += f"<b>├ 🔗 Source :</b> <a href='{task.listener.message.link}'>Link</a>\n"
